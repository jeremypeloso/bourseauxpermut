    ...
    <div className="w-full text-[#141A26]">
      ...
      <section className="relative isolate min-h-[calc(100svh-68px)] flex flex-col bg-[#0B1426]">
        ...
        <div className="relative border-t border-white/10 bg-[#0B1426]">
          <W cls="grid grid-cols-2 md:grid-cols-4 divide-x divide-white/10">
            {[['Identité', 'masquée jusqu\'à l\'accord'], ['Carte pro', 'lue puis détruite'], ['Gratuit', 'pour commencer'], ['Fondateur', 'ancien policier']].map(([b, s]) => <div key={b} className="py-5 px-5 first:pl-0"><b className="block text-white text-[15px]">{b}</b><span className="text-[13px] text-white/55">{s}</span></div>)}
          </W>
        </div>
      </section>
