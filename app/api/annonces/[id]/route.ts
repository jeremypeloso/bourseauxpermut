...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: a } = await admin.from('annonces').select('*, services(ville, departement, lat, lng)').eq('id', params.id).eq('institution', moi.institution).eq('statut', 'active').maybeSingle();
  ...
  if (!visible) {
    const { data: rows } = await admin.from('annonces').select('*, services(ville, departement, lat, lng)').eq('institution', moi.institution).eq('statut', 'active');
    const top = (rows ?? []).sort((x, y) => score(y) - score(x)).slice(0, 3).map(x => x.id);
    visible = top.includes(a.id);
  }
  const { data: sim } = await admin.from('annonces').select('*, services(ville, departement, lat, lng)').eq('institution', moi.institution).eq('statut', 'active').neq('id', a.id).limit(20);
