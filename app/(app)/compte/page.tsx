...
export default async function Page() {
  ...
  const { data: annonce } = await sb.from('annonces').select('id, statut, mise_en_avant_jusqua, created_at').eq('profil_id', user.id).eq('statut', 'active').eq('demo', false).maybeSingle();
