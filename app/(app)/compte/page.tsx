...
export default async function Page() {
  ...
  const { data: profil } = await sb.from('profils').select('institution, corps, grade, type_service, anciennete_poste_mois, depart_des, accepte_cycles, verifie_carte, verifie_mail_pro, verifie_le, premium_jusqua, created_at, services!profils_service_id_fkey(libelle, ville, departement)').eq('id', user.id).maybeSingle();
