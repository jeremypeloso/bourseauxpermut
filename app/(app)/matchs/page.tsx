...
export default async function Matchs() {
  ...
  const [{ data: profil }, { data: rows }, { data: ignorees }] = await Promise.all([
    sb.from('profils').select('premium_jusqua, verifie_carte, verifie_mail_pro').eq('id', user.id).maybeSingle(),
    sb.from('v_mes_correspondances').select('*').order('score', { ascending: false }),
    sb.from('correspondances_ignorees').select('correspondance_id'),
  ]);
